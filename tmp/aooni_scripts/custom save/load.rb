#==============================================================================
# 
# ?\ Yanfly Engine Ace - Ace セーブ Engine v1.03
...
# 2011.12.26 - Compatibility Update: はじめから+
...
# itself. This will in turn make the save command from the Main メニュー always
...
module YEA
  module SAVE
    ...
    SLOT_NAME = "ファイル %s"  # How the file slots will be named.
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ACTION_LOAD   = "ロード"           # Text used for loading games.
    ACTION_SAVE   = "セーブ"           # Text used for saving games.
    ...
    LOAD_HELP   = "ロードs the data from the saved game."
    SAVE_HELP   = "セーブs the current progress in your game."
    DELETE_HELP = "Deletes all data from this save file."
    
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    # - ステータス Window Settings -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    # to be displayed.
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    EMPTY_TEXT = "いいえ セーブ Data"      # Text used when no save data is present.
    PLAYTIME   = "Playtime: "  # Text used for total playtime.
    TOTAL_SAVE = "Total セーブs: "     # Text used to indicate total saves.
...
class Window_メニューCommand < Window_Command
...
end # Window_メニューCommand

#==============================================================================
# ?! Window_ファイルList
#==============================================================================

class Window_ファイルList < Window_Selectable
  ...
  def current_item_enabled?
    ...
    return false if header.nil? && SceneManager.scene_is?(Scene_ロード)
    return true
  end
...
class Window_ファイルステータス < Window_Base
...
end # Window_ファイルステータス

#==============================================================================
# ?! Window_ファイルAction
#==============================================================================

class Window_ファイルAction < Window_HorzCommand
  ...
  def save_enabled?
    return false if @header.nil? && SceneManager.scene_is?(Scene_ロード)
    return false if SceneManager.scene_is?(Scene_ロード)
...
end # Window_ファイルAction

#==============================================================================
# ?! Scene_ファイル
#==============================================================================

class Scene_ファイル < Scene_メニューBase
  ...
  def create_file_window
    ...
    @file_window = Window_ファイルList.new(0, wy)
  ...
  def create_action_window
    ...
    wy = @help_window.height
    @action_window = Window_ファイルAction.new(wx, wy, @file_window)
  ...
  def create_status_window
    ...
    @status_window = Window_ファイルステータス.new(wx, wy, @file_window)
  end
  ...
  def on_file_ok
    ...
    index = SceneManager.scene_is?(Scene_ロード) ? 0 : 1
